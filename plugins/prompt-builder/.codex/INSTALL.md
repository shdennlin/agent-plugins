...
Install the prompt-builder plugin for use with Codex via native skill discovery.
...
ln -s ~/.codex/shdennlin-agent-plugins/plugins/prompt-builder/skills ~/.agents/skills/prompt-builder
```
...
cmd /c mklink /J "$env:USERPROFILE\.agents\skills\prompt-builder" "$env:USERPROFILE\.codex\shdennlin-agent-plugins\plugins\prompt-builder\skills"
```
...
```bash
ls -la ~/.agents/skills/prompt-builder/
# Should list: prompt-builder/
```
...
```bash
rm ~/.agents/skills/prompt-builder
...
```powershell
cmd /c rmdir "$env:USERPROFILE\.agents\skills\prompt-builder"
