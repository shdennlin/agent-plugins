...
Read all spec files/folders provided in the prompt. If a folder is given, use Glob to find all relevant files within it (`.md`, `.txt`, `.yaml`, `.json`).
...
All git commands must run from the working directory provided in the prompt. Prefix all Bash commands with `cd <working directory> &&`.

### Step 3: Verify Git Repository

```bash
cd <working directory> && git rev-parse --is-inside-work-tree 2>&1
```
...
```bash
cd <working directory> && git diff <base>...HEAD --stat
```

**If base branch is "auto-detect":**
```bash
cd <working directory> && git rev-parse --abbrev-ref HEAD
cd <working directory> && git rev-parse --verify develop 2>/dev/null && echo "develop exists" || echo "no develop"
cd <working directory> && git rev-parse --verify main 2>/dev/null && echo "main exists" || echo "no main"
```
...
```bash
cd <working directory> && git diff <strategy> --stat
```

**For small diffs (< 20 files, < 500 lines changed):**
```bash
cd <working directory> && git diff <strategy>
```
...
```bash
cd <working directory> && git diff <strategy> -- <relevant-file-1> <relevant-file-2>
```
...
```bash
cd <working directory> && git diff --cached --stat
cd <working directory> && git diff --cached
```
...
2. **[HIGH]** `src/routes.ts` | `createUser`
   - **Context:** Spec §3.2 requires email, password (min 8 chars), and name fields
