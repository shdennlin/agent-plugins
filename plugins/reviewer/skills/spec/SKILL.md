...
Always dispatch `reviewer:spec-orchestrator` with parameters (paths, fix_enabled, max_iterations, angles, codebase_context, review-angles template content). The orchestrator fans out per angle in parallel and decides whether to also run the composition angle based on whether the scope contains multiple independent spec units. When `fix_enabled` is true it also runs Steps 5–7, escalating only the issues that need design judgment.
