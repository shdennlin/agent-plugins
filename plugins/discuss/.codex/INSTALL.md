...
Install the discuss plugin for use with Codex via native skill discovery.
...
ln -s ~/.codex/shdennlin-agent-plugins/plugins/discuss/skills ~/.agents/skills/discuss
```
...
cmd /c mklink /J "$env:USERPROFILE\.agents\skills\discuss" "$env:USERPROFILE\.codex\shdennlin-agent-plugins\plugins\discuss\skills"
```
...
```powershell
cmd /c rmdir "$env:USERPROFILE\.agents\skills\discuss"
