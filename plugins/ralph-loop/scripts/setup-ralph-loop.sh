...
# Priority: CLAUDE_SESSION_ID (from our SessionStart hook) > CLAUDE_CODE_SESSION_ID (from Claude Code)
if [[ -n "${CLAUDE_SESSION_ID:-}" ]]; then
  ...
  echo "" >&2
  echo "   This usually means the plugin was installed mid-session" >&2
  echo "   and the SessionStart hook has not fired yet." >&2
  echo "" >&2
  echo "   Fix: Restart your Claude Code session (exit and re-open)." >&2
  echo "   The SessionStart hook will fire on the next session start." >&2
  exit 1
fi
